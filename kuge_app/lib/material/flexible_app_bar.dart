...
class FlexibleDetailBar extends StatelessWidget {
  ...
  static InheritedWidget? percentage(BuildContext context) {
    InheritedWidget? value = context.dependOnInheritedWidgetOfExactType(aspect: _FlexibleDetail);
    return value;
  }

  const FlexibleDetailBar({super.key, required this.content, required this.builder, required this.background});

  @override
  Widget build(BuildContext context) {
    final FlexibleSpaceBarSettings? settings = context.dependOnInheritedWidgetOfExactType<FlexibleSpaceBarSettings>(aspect: FlexibleSpaceBarSettings);
...
class FlexShadowBackground extends StatelessWidget {
  ...
  const FlexShadowBackground({super.key, required this.child});

  @override
  Widget build(BuildContext context) {
    var t = FlexibleDetailBar.percentage(context);
    t = (Curves.ease.transform(t as double) / 2 + 0.2) as InheritedWidget?;
    return Container(
      foregroundDecoration: BoxDecoration(color: Colors.black.withOpacity(t as double)),
      child: child,
    );
  }
}
